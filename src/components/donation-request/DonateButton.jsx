...
export default function DonateButton({ requestId, user, status }) {
  ...
  const confirm=async()=>{setBusy(true); const {response,data}=await apiRequest(`/api/donation-requests/${requestId}/status`,{method:"PATCH",body:JSON.stringify({status:"inprogress"})}); setBusy(false); if(!response.ok)return toast.error(data.error||"Unable to confirm donation."); toast.success("Donation confirmed."); setOpen(false); router.refresh();};
